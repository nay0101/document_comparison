...
def group_by_section(doc_json: Dict[str, Any]) -> List[Dict[str, str]]:
    # First pass: Build sections without footnotes and collect all footnotes
    pairs = []
    current = None
...
def chunks_document(pdf_path: Path, pages_per_chunk: int, pages_limit) -> None:
...
def iter_image_jobs(
    ...
    if chunks_dir is None:
        chunks_dir = chunks_document(pdf_path, pages_per_chunk, pages_limit)
    else:
        chunks_dir = Path(chunks_dir)

    for idx, p in enumerate(sorted(chunks_dir.glob("*.pdf"))):
        doc = pymupdf.open(str(p))
        outdir = chunks_dir.parent / f"{chunks_dir.stem}_images"
        ...
        for i, page in enumerate(doc, start=1):
            ...
            pth = outdir / f"image_{idx:04d}_{i}.png"
            pix.save(pth)
...
def iter_pdf_chunk_jobs(
    pdf_path: Path, chunks_dir: Path | None, pages_per_chunk: int, pages_limit: int = 0
) -> Iterable[Dict[str, Any]]:
    if chunks_dir is None:
    ...
    try:
        if mode == "pdf":
            data = call_openai_on_pdf(
                client, model, job["path"], user_template, reasoning, verbosity
            )
        else:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_job):
            ...
            try:
                ...
                print(f"[error] Exception for {job['path'].name}: {e}")
                chunk_results[job["path"].name] = {"pages": [], "usage": None}

    # Reconstruct pages in the correct order based on chunk filenames
    for job in jobs:
        chunk_name = job["path"].name
        if chunk_name in chunk_results:
            data["data"] = chunk_results[chunk_name]
            all_pages.extend(data["data"].get("pages", []))
